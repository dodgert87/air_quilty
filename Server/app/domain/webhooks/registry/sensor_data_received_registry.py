...
from pydantic import SecretStr, AnyHttpUrl, TypeAdapter


class SensorDataReceivedWebhookRegistry(WebhookRegistryInterface):
    _webhooks: List[WebhookConfig] = []

    @classmethod
    async def load(cls, session: AsyncSession) -> None:
        db_webhooks = await get_active_webhooks_by_event(session, WebhookEvent.SENSOR_DATA_RECEIVED.value)

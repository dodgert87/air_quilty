...
class SensorStatusChangedProcessor(WebhookProcessorInterface[SensorOut]):
    ...
    async def load(self, session: AsyncSession) -> None:
        db_webhooks = await get_active_webhooks_by_event(session, WebhookEvent.SENSOR_STATUS_CHANGED.value)
    ...
    async def handle(self, payload: SensorOut, session: AsyncSession) -> None:
        ...
        for webhook in self._webhooks:
            try:
                logger.info("[SENSOR_STATUS_CHANGED] Dispatching webhook | sensor_id=%s | target=%s", payload.sensor_id, webhook.target_url)
                await send_webhook(session, webhook, payload_dict)
            except Exception as e:
                logger.exception("[SENSOR_STATUS_CHANGED] Failed to send webhook | sensor_id=%s | target=%s", payload.sensor_id, webhook.target_url)
